...
class LSTM_jordan(nn.Module):
    ...
    def __init__(self, input_dim,
        ...
        self.lstm = nn.LSTM(self.input_dim + self.n_jordan_params, self.hidden_dim, self.num_layers, bias=self.lstm_bias)
    ...
    def forward(self, input):
        ...
        if self.n_jordan_params == 3:
            p3 = torch.zeros_like(input) + self.jordan[:, 2]
            extended_input = torch.cat((input, p1, p2, p3), dim=2)
        else:
